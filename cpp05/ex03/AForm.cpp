...
/*   Updated: 2023/02/28 16:47:47 by ialinaok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	AForm::checkExecuteRequirements(Bureaucrat const & executor) const {
	...
	try {
		...
		std::cout << RED << "Cannot execute " << YELL << this->getName() << D << ": executor's grade [" << BLA << executor.getGrade() << D << "]" << RED << " too low" << D << std::endl;
		return (false);
	}
	return (true);
}
